...
func (s *slugBuild) writeRunDockerfile(sourceDir, packageName string, envs map[string]string) error {
	runDockerfile := `
	 ...
	 ENV VERSION=%s
	`
	logrus.Debugf("cacheDir:%v, from:%v, packageName:%v, Dir(slugPackage):%v", sourceDir, chaos.RUNNERIMAGENAME, packageName, path.Dir(packageName))
	result := util.ParseVariable(fmt.Sprintf(runDockerfile, chaos.RUNNERIMAGENAME, packageName, s.re.DeployVersion), envs)
