...
func TestMasterLock(t *testing.T) {
	master1, err := CreateMasterLock(nil, "/wutong/appruntimesyncmaster", "127.0.0.1:1", 10)
	if err != nil {
		t.Fatal(err)
	}
	master1.Start()
	defer master1.Stop()
	master2, err := CreateMasterLock(nil, "/wutong/appruntimesyncmaster", "127.0.0.1:2", 10)
	if err != nil {
		t.Fatal(err)
	}
	master2.Start()
