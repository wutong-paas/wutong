...
type wutongsslcProvisioner struct {
...
// NewWutongsslcProvisioner creates a new Wutong statefulset share volume provisioner
func NewWutongsslcProvisioner(kubecli kubernetes.Interface, store store.Storer) controller.Provisioner {
	if os.Getenv("ALLINONE_MODE") == "true" {
		return &wutongsslcProvisioner{
	...
	return &wutongsslcProvisioner{
		name:    "wutong.io/provisioner-sslc",
		kubecli: kubecli,
		store:   store,
	}
}
...
func (p *wutongsslcProvisioner) selectNode(ctx context.Context, nodeOS, ignore string) (*v1.Node, error) {
	...
	for _, node := range allnode.Items {
		...
		if strings.Contains(ignore, node.Name) {
			logrus.Debugf("[wutongsslcProvisioner] [selectNode] ignore node %s based on %s", node.Name, ignore)
			continue
		}
...
func (p *wutongsslcProvisioner) Provision(options controller.VolumeOptions) (*v1.PersistentVolume, error) {
	logrus.Debugf("[wutongsslcProvisioner] start creating PV object. paramters: %+v", options.Parameters)
	...
	logrus.Infof("create wutongsslc pv %s for pvc %s", pv.Name, options.PVC.Name)
	return pv, nil
}

// Delete removes the storage asset that was created by Provision represented
// by the given PV.
func (p *wutongsslcProvisioner) Delete(volume *v1.PersistentVolume) error {

	return nil
}

func (p *wutongsslcProvisioner) Name() string {
	return p.name
}
